...
export async function handleAuthPopupRequest(request: Request): Promise<Response> {
  ...
  const next = safeNextPath(url.searchParams.get("callbackURL"));
...
function completionResponse(message: PopupMessage, next = "/account"): Response {
  return new Response(completionHtml(message, next), {
    status: 200,
    headers: {
      "content-type": "text/html; charset=utf-8",
...
/** Minimal HTML: postMessage the token to the opener, then continue. */
function completionHtml(message: PopupMessage, next: string): string {
...
<title>${ok ? "Signing in…" : "Sign-in didn't finish"}</title>
<style>
  html,body{margin:0;min-height:100%;background:#0b0b0c;color:#d4d4d8;
    font:16px/1.5 -apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,Helvetica,Arial,sans-serif}
  main{min-height:100vh;display:grid;place-items:center;padding:1.5rem;text-align:center}
  a,button{display:inline-block;margin-top:1rem;padding:.85rem 1.25rem;border:0;border-radius:6px;
    background:#4ade80;color:#052e16;font:inherit;font-weight:600;text-decoration:none;cursor:pointer}
  .err{color:#fca5a5}
</style>
</head>
<body>
<main>
  <p id="status">${ok ? "Signing you in…" : "Sign-in didn't finish. Use email, or try Google / X again."}</p>
  <a id="go" href=${dest}>${ok ? "Continue to your desk" : "Back to sign in"}</a>
</main>
...
(function () {
  ...
  if (msg.token) {
    try { sessionStorage.setItem("grok-auth.bearer-token", msg.token); } catch (e) {}
    try { localStorage.setItem("grok-auth.bearer-token", msg.token); } catch (e) {}
  }
  var go = document.getElementById("go");
  if (go) go.setAttribute("href", msg.token ? next : "/login");
  try {
  ...
  setTimeout(function () {
    ...
    window.location.replace(msg.token ? next : "/login");
  }, 350);
})();
</script>
</body>
