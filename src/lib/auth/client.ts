 ...
 */
function openSignInPopup(providerId: string, callbackURL = "/account"): Window | null {
  const origin = window.location.origin;
  const url = `${origin}/auth/popup?providerId=${encodeURIComponent(providerId)}&callbackURL=${encodeURIComponent(callbackURL)}`;
