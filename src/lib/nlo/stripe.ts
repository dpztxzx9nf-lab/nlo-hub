...
export async function createCoinCheckout(input: {
  ...
  const product = input.ign
    ? `${input.coins.toLocaleString()} NLO coins for ${input.ign} — ${input.name}`
    : `${input.coins.toLocaleString()} NLO coins — ${input.name}`;
  const session = await stripePost("checkout/sessions", {
    ...
    ...(input.ign ? { "metadata[ign]": input.ign } : {}),
    ...(input.uuid ? { "metadata[uuid]": input.uuid } : {}),
  });
